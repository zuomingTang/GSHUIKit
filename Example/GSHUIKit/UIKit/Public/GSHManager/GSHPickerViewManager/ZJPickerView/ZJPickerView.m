...
@property (nonatomic, copy) void(^completion)(NSString * _Nullable  selectContent , NSArray *selectRowArray); // select content
...
- (void)userAction:(UIButton *)sender
{
    ...
    // click sure
    if (sender.tag == 1) {
        NSMutableArray *selectRowArray = [NSMutableArray array];
        NSMutableString *selectString = [[NSMutableString alloc] init];
        for (NSUInteger i = 0; i < self.component; i++) {
            NSInteger selectRow = [self.pickerView selectedRowInComponent:i];
            [selectRowArray addObject:@(selectRow)];
        ...
        // completion callback
        if (self.completion) {
           self.completion(selectString , selectRowArray);
        }
    } else {
        if (self.cancelBlock) {
            self.cancelBlock();
...
+ (void)zj_showWithDataList:(nonnull NSArray *)dataList
               propertyDict:(nullable NSDictionary *)propertyDict
                 completion:(nullable void(^)(NSString * _Nullable selectContent , NSArray *selectRowArray))completion
{
    // no data
    ...
    [self sharedView].pickerView.dataSource = [self sharedView];
    [self sharedView].pickerView.delegate = [self sharedView];
                 ...
                 completion:(nullable void(^)(NSString * _Nullable selectContent , NSArray *selectRowArray))completion
{
    ...
    if (cancelBlock) {
        [self sharedView].cancelBlock = cancelBlock;
    }
...
+ (UIPickerView*_Nonnull)zj_showWithDelegate:(nullable id<UIPickerViewDataSource, UIPickerViewDelegate>)delegate
                                propertyDict:(nullable NSDictionary *)propertyDict
                                  completion:(nullable void(^)(NSString * _Nullable selectContent , NSArray * _Nullable selectRowArray))completion{
    // no data
    if (!delegate) {
        return nil;
    }
    ...
    [[self sharedView].propertyDict addEntriesFromDictionary:propertyDict];
    [[self sharedView] updateCustomProperiesSetter];
    [self sharedView].pickerView.dataSource = delegate;
    [self sharedView].pickerView.delegate = delegate;
    ...
    return [self sharedView].pickerView;
}
...
- (void)scrollToSelectedRow
{
    ...
    if (selectedContent.length && ![selectedContent isEqualToString:@""]) {
        __weak typeof(self) weakself = self;
        NSArray *selectedContentArray = [selectedContent componentsSeparatedByString:@","];
        if (selectedContentArray.count != self.component) {
            return;
        }
        ...
        for (NSUInteger i = 0; i < self.component; i++) {
            ...
            NSString *selectedContentItem = selectedContentArray[i];
            if (componentArray.count) {
                [componentArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    ...
                    if (![title isEqualToString:@""]) {
                        NSRange range = [selectedContentItem rangeOfString:title];
